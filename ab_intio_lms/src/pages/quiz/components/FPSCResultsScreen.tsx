...
export default function FPSCResultsScreen({
  ...
  const calculatedPercentage = ((totalScore / totalQuestions) * 100).toFixed(1)
  ...
  return (
    <div className="card relative overflow-hidden p-8 lg:p-10 border-2 flex flex-col items-center" 
      style={{ borderColor: hasPassed ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)' }}>
      
      <div className="absolute top-0 left-0 right-0 py-2.5 text-white font-black tracking-widest text-[10px] uppercase shadow-sm text-center"
        style={{ background: hasPassed ? 'var(--color-success)' : 'var(--color-error)' }}>
        {hasPassed ? 'Preparation Exam Passed' : 'Preparation Exam Failed'}
      </div>
      ...
      <h1 className="text-3xl font-extrabold mb-1" style={{ color: 'var(--text-primary)' }}>Final Examination Results</h1>
      <p className="text-xs mb-6" style={{ color: 'var(--text-muted)' }}>
        {timeLeft === 0 ? "Time has expired." : "Assessment completed successfully."} Qualifying standard is {activeTest.passingScore}% ({Math.round(totalQuestions * activeTest.passingScore / 100)} Marks).
      </p>
      ...
      <div className="grid grid-cols-3 gap-4 w-full mb-6">
        <div className="bg-[var(--bg-surface)] p-4 rounded-2xl border-l-4 border flex flex-col gap-1 shadow-sm" style={{ borderColor: 'var(--border-color)', borderLeftColor: 'var(--color-success)' }}>
          <span className="text-[10px] uppercase font-bold tracking-wider" style={{ color: 'var(--text-muted)' }}>Correct</span>
        ...
        <div className="bg-[var(--bg-surface)] p-4 rounded-2xl border-l-4 border flex flex-col gap-1 shadow-sm" style={{ borderColor: 'var(--border-color)', borderLeftColor: 'var(--color-error)' }}>
          <span className="text-[10px] uppercase font-bold tracking-wider" style={{ color: 'var(--text-muted)' }}>Incorrect</span>
        ...
        <div className="bg-[var(--bg-surface)] p-4 rounded-2xl border-l-4 border flex flex-col gap-1 shadow-sm" style={{ borderColor: 'var(--border-color)', borderLeftColor: 'var(--color-warning)' }}>
          <span className="text-[10px] uppercase font-bold tracking-wider" style={{ color: 'var(--text-muted)' }}>Skipped</span>
      ...
      <div className="bg-[var(--bg-surface)] rounded-xl p-5 w-full text-left border mb-6 flex flex-col gap-4" style={{ borderColor: 'var(--border-color)' }}>
        {sectionBreakdowns.map((sec) => (
          <div key={sec.id} className="flex flex-col gap-1.5 pb-3 border-b" style={{ borderColor: 'var(--border-color)' }}>
            <div className="flex justify-between items-center text-xs">
              <span className="font-bold" style={{ color: 'var(--text-primary)' }}>{sec.name} ({activeTest.sections.find(s => s.id === sec.id)?.questions.length} Marks)</span>
        ...
        <div className="flex justify-between items-center pb-2 border-b text-red-400" style={{ borderColor: 'var(--border-color)' }}>
          <span className="text-xs font-semibold flex items-center gap-1.5">
            <AlertCircle className="w-3.5 h-3.5" /> Negative Penalty Deductions
          </span>
        ...
        <div className="flex justify-between items-end pt-2">
          <div>
            <span className="text-sm font-bold block" style={{ color: 'var(--text-primary)' }}>Aggregate Score</span>
            <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Total out of {totalQuestions} Marks</span>
          </div>
          <div className="text-right">
      ...
      {weakestSection && (
        <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-5 mb-6 text-left w-full">
          <h3 className="text-yellow-500 font-bold text-xs uppercase flex items-center gap-2 mb-2">
            <Target className="w-4 h-4" /> Syllabus Diagnostic Analysis
          </h3>
          <p className="text-xs leading-normal mb-2" style={{ color: 'var(--text-secondary)' }}>
            Your weakest performance was in <strong style={{ color: 'var(--text-primary)' }}>{weakestSection.name}</strong>, where you scored {weakestSection.score.toFixed(1)}%.
          </p>
          <p className="text-xs italic" style={{ color: 'var(--text-muted)' }}>
            <BookOpen className="w-3.5 h-3.5 inline mr-1" /> Core Recommendation: {activeTest.sections.find(s => s.id === weakestSection.id)?.studyTip}
          </p>
        </div>
      )}
      ...
      <button
        onClick={onRetake}
        className="w-full flex items-center justify-center space-x-2 text-white font-bold py-3.5 px-8 rounded-xl transition-all duration-200 shadow-md"
        style={{ background: 'var(--color-primary)' }}
      >
        <RotateCcw className="w-5 h-5" />
        <span>Retake Preparation Examination</span>
      </button>
    </div>
  )
}
